...
def draw_spring(axis, start_x: float, end_x: float, center_y: float = 0.5,
                number_of_coils: int = 10, coil_amplitude: float = 0.08) -> Line2D:
    """
    Draw a helical spring between two horizontal points.

    Parameters:
        axis: matplotlib axis to draw on
        start_x: left x-coordinate (wall side)
        end_x: right x-coordinate (mass side)
        center_y: vertical center of spring
        number_of_coils: how many coils to draw
        coil_amplitude: height of each coil wave
    """
    points_per_coil = 20
    total_points = number_of_coils * points_per_coil + 1

    # Generate sinusoidal wave for spring shape
    wave_parameter = np.linspace(0, number_of_coils * 2 * np.pi, total_points)
    spring_length = end_x - start_x

    x_coordinates = start_x + (wave_parameter / (number_of_coils * 2 * np.pi)) * spring_length
    y_coordinates = center_y + coil_amplitude * np.sin(wave_parameter)

    # Flatten the ends of the spring (attachment points)
    flat_end_points = 5
    y_coordinates[:flat_end_points] = center_y
    y_coordinates[-flat_end_points:] = center_y

    spring_line, = axis.plot(
        x_coordinates, y_coordinates,
        color=COLORS['spring'], linewidth=2.5, solid_capstyle='round'
    )
    return spring_line
...
def create_static_plots(solution: dict) -> plt.Figure:
    ...
    position = solution['x']
    velocity = solution['v']
    kinetic_energy = solution['KE']
    potential_energy = solution['PE']
    total_energy = solution['E_total']
    ...
    # Position
    axes[0, 0].plot(time_array, position, color=COLORS['position'], linewidth=1.5)
    ...
    # Velocity
    axes[0, 1].plot(time_array, velocity, color=COLORS['velocity'], linewidth=1.5)
    ...
    # Phase space
    axes[1, 0].plot(position, velocity, color=COLORS['position'], linewidth=1.5)
    axes[1, 0].plot(position[0], velocity[0], 'go', markersize=10, label='Start')
    axes[1, 0].plot(position[-1], velocity[-1], 'rx', markersize=10, label='End')
    ...
    # Energy
    axes[1, 1].plot(time_array, total_energy, color=COLORS['energy_total'], linewidth=2, label='Total')
    axes[1, 1].plot(time_array, kinetic_energy, color=COLORS['energy_kinetic'], linewidth=1.5, label='Kinetik', alpha=0.8)
    axes[1, 1].plot(time_array, potential_energy, color=COLORS['energy_potential'], linewidth=1.5, label='Potensial', alpha=0.8)
    axes[1, 1].axhline(y=total_energy[0], color=COLORS['text'], linestyle=':', alpha=0.5, label='E₀')
